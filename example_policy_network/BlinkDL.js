    ...
    function sgemm(m, n, k, a, b, c) {
        ...
        for (;
            ...
            for (j = 0;
                ...
                xfinal = x + (k << 2) | 0;
                y = b + (j << 2) | 0;
                z = c + ((imul(i, n) | 0) + j << 2) | 0;

                value = fround(0.0);

                for (;
                    (x | 0) < (xfinal | 0); x = x + 16 | 0, y = y + nnn | 0) {
                    value = fround(value + fround(buf[x >> 2] * buf[y >> 2]));
                    value = fround(value + fround(buf[x + 4 >> 2] * buf[y + nn >> 2]));
                    value = fround(value + fround(buf[x + 8 >> 2] * buf[y + nn + nn >> 2]));
                    value = fround(value + fround(buf[x + 12 >> 2] * buf[y + nn + nn + nn >> 2]));
                }

                for (;
                    (x | 0) < (xfinal | 0); x = x + 4 | 0, y = y + nn | 0) {
                    value = fround(value + fround(buf[x >> 2] * buf[y >> 2]));
                }
...
function im2col3x3pad1(from, to, channel, sz) {
    var desc = 0;
    var src = 0;
    const sz2 = sz * sz;
    for (var i = 0; i < channel; i++) {
        for (var j = sz + 1; j < sz2; j++) {
            to[desc + j] = (j % sz != 0) ? from[src + j - sz - 1] : 0;
        }
        desc += sz2;
        ...
        for (var j = sz; j < sz2 - 1; j++) {
            to[desc + j] = (j % sz != (sz - 1)) ? from[src + j - sz + 1] : 0;
        }
        desc += sz2;
        for (var j = 1; j < sz2; j++) {
            to[desc + j] = (j % sz != 0) ? from[src + j - 1] : 0;
        }
        desc += sz2;
        for (var j = 0; j < sz2; j++) {
        ...
        for (var j = 0; j < sz2 - 1; j++) {
            to[desc + j] = (j % sz != (sz - 1)) ? from[src + j + 1] : 0;
        }
        desc += sz2;
...
BlinkArray.prototype.Convolution = function (outChannel, kerSize) {
    ...
    if (kerSize == 3)
        im2col3x3pad1(this.data, b, this.nChannel, Math.sqrt(n))
    else if (kerSize == 1) {
        for (var i = 0; i < k * n; i++) {
            b[i] = this.data[i];
        }
    ...
    var out = new Float32Array(gHeap, gFloatSize * (m * k + k * n), m * n);
    ...
    if (this.backend == 'asm.js')
        gBackend.sgemm(m, n, k, a.byteOffset, b.byteOffset, out.byteOffset);
    ...
    this.paramIndex += m; // FIXME: should avoid out-of-range
    this.data = out;
    this.nChannel = m;
    return this;
}
